...
const getGradientStop = (ratio: number) => {
    ...
    const leftMatch = LEFT_COLOR.match(/.{1,2}/g);
    const c0 = (leftMatch ? leftMatch : ["00", "00", "00"]).map(
    (oct) => parseInt(oct, 16) * (1 - ratio)
    );
    const rightMatch = RIGHT_COLOR.match(/.{1,2}/g);
    const c1 = (rightMatch ? rightMatch : ["00", "00", "00"]).map(
